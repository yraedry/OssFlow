...
public record PatchNoteRequest(
        @Size(max = 255) @Pattern(regexp = "^[^<>]*$", message = "No se permiten caracteres HTML") String title,
        @Size(max = 50000) String bodyMarkdown,
        @Size(max = 50) @Pattern(regexp = "^[^<>]*$", message = "No se permiten caracteres HTML") String targetType,
        Long targetId,
        List<@Size(max = 100) @Pattern(regexp = "^[^<>]*$", message = "No se permiten caracteres HTML") String> tags
) {}
